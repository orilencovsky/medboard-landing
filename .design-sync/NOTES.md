...
- **RTL (the Hebrew root `index.html`) was not modeled.** The original 14 components are LTR-only, matching `en/index.html`. The Hebrew page mirrors the same design but was not given a `dir` prop or RTL variant — if the design agent needs to build RTL screens with this DS, that's new work, not a gap-fill. `LoopSection` is the one exception: it takes `dir` and reverses the ring's flow direction with it.
- **`LoopSection` has no counterpart on the shipped page.** It is the only component here that is a *proposal* rather than a port — a closed-loop section built for the differentiation rewrite, covering two stages (`דק הפערים`, `ספריה`) that are roadmap, not product. Those carry `state: 'soon'`, which renders the stage rail's own upcoming-node treatment plus a roadmap pill; don't drop the marker when composing it. Its readiness figure and heat map are illustrative props, not live data.
